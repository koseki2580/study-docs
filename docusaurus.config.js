...
WriteFile("./src/components/HomepageFeatures/content.js",WriteContents(contents,"content"));
    ...
    [
      ...
      ({
        docs: {
          ...
          editUrl:
            'https://github.com/koseki2580/study-docs/edit/tree/main/',
        },
        theme: {
...
function getFiles(){
  ...
  for(let i = 0; i < parentdatas.length; ++i){
    ...
    // サイドバーのコンフィグを設定する
    sidebar_js[parentName + "Sidebar"] = [{type: 'autogenerated', dirName: parentdatas[i]}];
    ...
    for (let j = 0; j < childrendatas.length; ++j){
      ...
      if (childrendatas[j] === "_category_.json"){
        // JSONオブジェクトを取得
        const jsonObject = JSON.parse(fs.readFileSync( baseUrl + parentdatas[i] + "/" + childrendatas[j], 'utf8'));
      ...
      if (childrenName.length >= 2){
        ...
        // navbarに子要素を入れる
        let navbaritem = {type: 'doc', to:`/docs/${parentName}/${childName}`, label:title, docId: `${parentName}/${childName}`}
        navparent.items.push(navbaritem);
        continue;
      }
      // フォルダーの場合（孫要素が存在する場合）
      let finaldatas = fs.readdirSync(baseUrl + parentdatas[i] + "/" + childrendatas[j] + "/");
      for (let k = 0; k < finaldatas.length; ++k){
        ...
        if (finaldatas[k] === "_category_.json"){
          // jsonファイルを読み込み
          const jsonObject = JSON.parse(fs.readFileSync( baseUrl + parentdatas[i] + "/" + childrendatas[j] + "/_category_.json", 'utf8'));
          ...
          // navbarに子要素を入れる
          let navbaritem = {type: 'doc', to:`/docs/${parentName}/${childName}`, label:jsonObject.label, docId: `${parentName}/${childName}/README`}
          navparent.items.push(navbaritem);
          break;
        }
      }
...
function WriteContents(contents,variable){
  let str = `const ${variable} = ${JSON.stringify(contents)}\nmodule.exports = ${variable};`
  return str;
}
