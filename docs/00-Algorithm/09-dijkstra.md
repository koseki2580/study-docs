...
探索対象頂点から 1 つ頂点を取り出す。取り出した頂点に隣接する頂点へのパスコストを比較し、経路テーブルが更新される場合はコスト更新を行い、探索対象頂点に頂点を追加する。
...
となり、3,6 番の経路テーブルの更新は行われない。4,7 番の経路テーブルは更新が発生するので探索対象頂点に追加する。この時点で 3 番への経路は全て確認したので現在のコストが最小となることがわかる。
