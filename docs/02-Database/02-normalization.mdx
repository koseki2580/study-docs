...
この関係表では{受注番号 1}と決まれば、{商品 A, 商品 B}または{材料 A, 材料 B}と一意に決定する。つまり、{受注番号}が決まれば、独立して{商品名}と{材料名}が決まるため、次の関係が存在する。

- {受注番号} $\rightarrow$$\rightarrow$ {商品名 | 材料名}

この関係を{受注番号} $\rightarrow$$\rightarrow$ {商品名}と{受注番号} $\rightarrow$$\rightarrow$ {材料名}
に分ける正規化を行う。
...
といったように{受注番号,　商品名, 材料名}が独立していないため、この関係を分解する。
