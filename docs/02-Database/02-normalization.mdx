...
自明な関数従属性とは X $\rightarrow$ Y である時に Y が X の部分集合である集合である時のことをいう。
{受注番号, 商品番号} $\rightarrow$ {商品番号}のような場合をさす。

スーパキーとは関係のタプルを一意に選択できるという条件のみを満たすキーのことをいう。
...
- {受注番号} $\rightarrow$$\rightarrow$ {商品名 | 材料名}

が存在する。この関係を{受注番号} $\rightarrow$$\rightarrow$ {商品名}と{受注番号} $\rightarrow$$\rightarrow$ {材料名}
に分ける正規化を行う。
...
![イメージ図](/img/svg/Database/normalization/normalization-9.drawio.svg "ベース図")

この関係表では{受注番号 1}に対する{商品名 A, 商品名 D}や{材料名 B}に対する{受注番号 1, 受注番号 2}などの様々な従属性が考えられる。
これらを分解する。

![イメージ図](/img/svg/Database/normalization/normalization-10.drawio.svg "第5正規化")
