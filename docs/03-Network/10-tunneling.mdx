...
最初の交換は通常 4 つのメッセージで構成されるが、シナリオ(AH なのか ESP なのかトンネルモードなのかトランスポートモードなのか)によって増えるが場合がある。
...
最初の交換に続くすべてのメッセージは IKE_SA_INIT で決定された暗号化アルゴリズムとキーを使用して暗号化されて保護されます。
...
Security Association - Initiator 2 ペイロードを使用して子 SA のネゴシエーションを開始する。
Security Association - Initiator 2 から始まるフィールドは、CREATE_CHILD_SA 交換の説明がされている。
